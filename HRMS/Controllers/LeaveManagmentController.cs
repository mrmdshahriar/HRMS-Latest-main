    ...
    public class LeaveManagmentController : Controller
    {
        ...
        public ActionResult AddLeaveRequest(LeaveRequest request)
        {
            try
            {
               ...
               // var AnnualLeaveSal = _hrms.AnnualLeaveSalaryCalculations.Where(x => x.EmpId == request.Employee).ToList();
                ...
                foreach(var x in EmployeeDataList)
                {
                    ...
                    if (request.Status == "Approved" && request.Encashment == true )
                    {
                        return Json(new { success = false, message = "Sorry! you are unable to apply for This Leave.", JsonRequestBehavior.AllowGet });
                    }
                ...
                if (request.Encashment == true)
                {
                    GetAnnualLeaveSalaryData(request.Employee, request);
                    //_hrms.AnnualLeaveSalaryCalculations.Add(annualCal);
                    // return Json(new { success = false, message = "Sorry! you are unable to apply more than once.", JsonRequestBehavior.AllowGet });
                }
        ...
        public void GetAnnualLeaveSalaryData(long id, LeaveRequest levReq)
        {
            
                var data = (from emp in _hrms.HrmEmployees.AsEnumerable()
                            join LevReq in _hrms.LeaveRequests on emp.Id equals LevReq.Employee into LevReqGroup
                            from lg in LevReqGroup.DefaultIfEmpty()
                            join cal in _hrms.SalaryCalculationHeaders on emp.Id equals cal.EmployeeId into calGroup
                            from cg in calGroup.DefaultIfEmpty()
                            join dept in _hrms.Departments on emp.DepartmentId equals dept.Id into deptGroup
                            from deptg in deptGroup.DefaultIfEmpty()
                            where emp.Id==id && cg?.SalaryDate?.Month == DateTime.Now.Month && cg?.SalaryDate?.Year == DateTime.Now.Year
                            select new
                            {
                                //Id = emp.Id,
                                EmployeeId = emp.Id,
                                EmployeeName = emp.FirstName + " " + emp.LastName,
                                Department = deptg?.Name,
                                DateOfJoining = emp.dteJoiningDate,
                                GrossMonthlySalary = cg?.TotalSalaryWithAllowances,
                                NoofPendingAnnualLeaves = lg?.PendingDays,
            ...
            if(data != null)
            {
                AnnualLeaveSalaryCalculation annualCal = new AnnualLeaveSalaryCalculation();
                annualCal.Department = data.Department;
                annualCal.EmpId = data.EmployeeId;
                annualCal.EmpName = data.EmployeeName;
                annualCal.DateOfJoining = data.DateOfJoining;
                annualCal.GrossMonthlySalary = data.GrossMonthlySalary;
                annualCal.NoOfPendingAnnualLeaves = levReq.PendingDays;
                annualCal.Tenure = data.Tenure;
                annualCal.CashOutMonth = levReq.CashOutDate;
                annualCal.Status = levReq.Status;
                annualCal.LeaveSalary = ((data.GrossMonthlySalary * 12) / 365) * levReq.PendingDays;
        ...
        public decimal GetTenure(DateTime? JoiningDate)
        {
            decimal tenure = 0;
            int employeeWorkedDays = (DateTime.Now - JoiningDate).Value.Days;
            tenure = ((decimal)employeeWorkedDays / 365);
            return tenure;
        }
        ...
        {
            AnnualLeaveSalaryCalculation anualCal = new AnnualLeaveSalaryCalculation();
            try
            {
        ...
        {
            var EmployeeDataObj = _hrms.LeaveRequests.Where(x => x.Employee == employeeId && x.Status == "Approved" && x.RejoiningDate == null).OrderByDescending(x => x.DateTo).FirstOrDefault();
        ...
        public ActionResult CheckAnnualLeavesAvailable(int employeeId, string leaveType)
        {
            ...
            LeaveRequest request = _hrms.LeaveRequests.FirstOrDefault();
            double eligibilityDays = 0;

            if (EmployeeObj.ProbationPeriod != null)
            {
                DateTime ProbationEndDate = (DateTime)EmployeeObj.dteJoiningDate?.Date.AddMonths((int)EmployeeObj.ProbationPeriod);
            ...
            var Encashable = (from lp in _hrms.LeavePolicies
                              join lt in _hrms.LeaveTypes
                              on lp.LeaveTypeId equals lt.Id
                              where lt.Name == leaveType
                              select new
                              {
                                  IsEncashable = lp.IsEncashable
                              }).FirstOrDefault();


            return Json(new { success = true, message = "Ok", EligibilityDays = eligibilityDays, Encashable = Encashable }, JsonRequestBehavior.AllowGet);
        }
